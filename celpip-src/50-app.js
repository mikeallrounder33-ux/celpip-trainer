...
/* ---------------- 50.9 SETTINGS ---------------- */
/* ---------------- 50.9a advanced (owner) settings ----------------
   Provider and API configuration is hidden from ordinary visitors: a stranger
   should not be invited to paste an API key into someone else's page, and the
   options are noise for someone who just wants to practise.

   This is CONCEALMENT, NOT SECURITY. The source is public and anyone who reads
   it can set the same flag. It keeps the interface clean; it does not protect
   anything, and nothing sensitive is stored here to protect. */
function advancedOn() { return DB.get('celpip_advanced', false) === true; }
function setAdvanced(v) { DB.set('celpip_advanced', !!v); }
(function checkAdvancedFlag() {
  try {
    if (/[?&]advanced=1/.test(location.search) || /^#advanced$/.test(location.hash)) {
      setAdvanced(true);
...
function renderSettings() {
  ...
  // Everything inside `adv` is owner-only.
  const adv = el('div');
  adv.style.display = ADV ? '' : 'none';

  adv.appendChild(el('label', { class: 'fl', text: 'Provider' }));
  ...
  if (!ADV) {
    // Public view: the only AI option offered is the one that needs no key and
    // sends nothing anywhere.
    prov.value = 'browser';
    c.appendChild(el('p', {
      class: 'small muted',
      html: 'Everything below is optional. The practice tests, the timers and the marking all work with no setup at all.'
    }));
  }
  c.appendChild(bWrap);
  c.appendChild(adv);

  adv.appendChild(el('label', { class: 'fl', style: 'margin-top:14px', text: 'API key' }));
  const key = el('input', { type: 'password', value: s.apiKey, placeholder: 'sk-…' });
  adv.appendChild(key);
  const keyNote = el('p', { class: 'tiny muted', style: 'margin-top:6px' });
  adv.appendChild(keyNote);

  adv.appendChild(el('label', { class: 'fl', style: 'margin-top:14px', text: 'Model' }));
  const model = el('input', { type: 'text', value: s.model || 'claude-sonnet-4-6' });
  adv.appendChild(model);
  const modelNote = el('p', { class: 'tiny muted', style: 'margin-top:6px' });
  adv.appendChild(modelNote);
  ...
  adv.appendChild(el('label', { class: 'fl', style: 'margin-top:14px', text: 'max_tokens per request' }));
  const mt = el('input', { type: 'number', value: s.maxTokens || 1000, min: '400', max: '8000' });
  adv.appendChild(mt);
  adv.appendChild(el('p', { class: 'tiny muted', text: '1000 is the specified default. Raise it if generated Part 5 discussions or Part 4 articles come back truncated (the app falls back to the offline bank when that happens).' }));
  ...
  if (ADV) c.appendChild(test);

  if (ADV) {
    const lock = el('div', { class: 'flagline', style: 'margin-top:16px' });
    lock.innerHTML = '<strong>Advanced settings are unlocked on this browser.</strong> ' +
      'Provider and API configuration are hidden from ordinary visitors — this is to keep the ' +
      'interface clean and to avoid inviting strangers to paste a key into someone else\'s page. ' +
      'It is not a security control: the source is public, so anyone who reads it can unlock the same view.';
    const lb = el('button', { class: 'btn ghost sm', style: 'margin-top:8px', text: 'Lock advanced settings' });
    lb.onclick = () => { setAdvanced(false); toast('Locked. Re-open with ?advanced=1 to unlock.', 'ok'); go('settings'); };
    lock.appendChild(lb);
    c.appendChild(lock);
  }
  wrap.appendChild(c);
  ...
  if (ADV) wrap.appendChild(dg);   // owner-only: noise for a visitor
